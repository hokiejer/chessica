...
impl Tree {
    ...
    pub fn iterative_alpha_beta_keep_depth_promote(&mut self, keep_depth: u8, depth: u8, mut min: i32, mut max: i32) -> i32 {
        let mut temp_score: i32 = 0;
        self.reset.conditionally_complete_move_initialization();
        for i in 1..(depth+1) {
            let mut move_count: u64 = 0;
            println!("i == {}",i);
            temp_score = self.alpha_beta_keep_depth_promote(keep_depth,i,min,max,&mut move_count);
...
mod tests {
    ...
    fn ab_iterative_keep_depth_stalemate_test() {
        ...
        let score = t.iterative_alpha_beta_keep_depth_promote(4, 8, SCORE_MAX, SCORE_MIN);
