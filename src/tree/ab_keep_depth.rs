...
impl Tree {
    ...
    pub fn alpha_beta_keep_depth(&mut self, keep_depth: u8, depth: u8, mut min: i32, mut max: i32) -> i32 {
        ...
        if depth == 0 {
            ...
            while self.add_next_child() {
                ...
                let new_depth = if keep_depth > 0 {
                    keep_depth - 1
                } else {
                    0
                };
                let mut temp_score: i32 = if keep_depth == 0 {
                    child.alpha_beta_in_place(depth-1,min,max)
                } else {
                    child.alpha_beta_keep_depth(new_depth,depth-1,min,max)
                };
